...
class Infracoes extends CI_Controller {
 ...
 function editar_responsavel_infracao(){
	...
	if($this->infracao_model->atualiza_resp($dados,$id)) {
		...
		$data['mensagem'] = 'Cadastro do Respons&aacute;vel Atualizado com Sucesso';
...
function atualizar_responsavel_infracao(){
	...
	if($this->infracao_model->add_resp($dados)) {
		$_SESSION["mensagemInfra"]  = 'Cadastro do Respons&aacute;vel Realizado com Sucesso';
	}else{	
...
function csv($result){
	...
	<td>Bandeira</td>
	<td>CNPJ</td>
	<td>C&oacute;digo da Infra&ccedil;&atilde;o</td>
	<td>Data Recebimento da Infra&ccedil;&atilde;o</td>
	<td>Tipo do Auto</td>
	<td>Classifica&ccedil;&atilde;o da Infra&ccedil;&atilde;o </td>
	<td>Motivo da Infra&ccedil;&atilde;o</td>
	<td>Breve Relato da Infra&ccedil;&atilde;o</td>
	<td>Compet&ecirc;ncia Inicio</td>
	<td>Compet&ecirc;ncia Fim</td>
	<td>Valor Principal</td>
	<td>Atualiza&ccedil;&atilde;o Monet&aacute;ria</td>
	<td>Multa</td>
	<td>Juros</td>
	<td>Total</td>
	<td>Valor Total Revisado</td>
	<td>Valor Efetivo Pago</td>
	<td>Data planejada fim processo</td>
	<td>Data de encerramento efetivo</td>
	<td>Observa&ccedil;&otilde;es</td>
	<td>Nome Respons&aacute;vel</td>
	<td>Data Envio ao Respons&aacute;vel</td>
	<td>Posicionamento Respons&aacute;vel</td>
	<td>Probabilidade &Ecirc;xito</td>
	<td>Status Infra&ccedil;&atilde;o</td>
	<td>Alterado Por </td>
	...
	if($isArray == 0){
		...
		$test .="<td class='hidden-phone' colspan='5'> Não H&aacute; Dados </td>";
		...
		foreach($result as $imovel){ 
			...
			$dadosObs = $this->infracao_model->buscaTodasObservacoes($idInfracao);		
			...
			$test .="<td >".utf8_decode($imovel->loja_estado)."</td>";
			$test .="<td >".utf8_decode($imovel->loja_cidade)."</td>";
			$test .="<td >".utf8_decode($imovel->descricao_bandeira)."</td>";
			$test .="<td >".utf8_decode($imovel->regional_desc)."</td>";
...
function listar(){
 ...
 function ativar(){
	...
	if($this->infracao_model->ativar($id)) {
		...
		$_SESSION["mensagemInfra"] = 'Infra&ccedil;&atilde;oo foi ativado';
 ...
 function excluir_resp(){
	...
	if($this->infracao_model->excluir_resp($id)) {
		...
		$msg = 'Respons&aacute;vel foi excluído';
  ...
  function excluir(){
	...
	if($podeExcluir[0]['total'] == 1){		
		...
		if($this->infracao_model->excluir($id)) {
			...
			$data['mensagem'] = 'Infra&ccedil;&atilde;oo foi inativada';
