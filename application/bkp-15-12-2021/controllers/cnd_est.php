...
//redirect('/iptu/listar', 'refresh');		 
 ...
 function enviar_pend(){
	...
	$extensao = str_replace('.','',strrchr($file, '.'));						
	$base = base_url();		        
	$config['upload_path'] = './assets/cnd_est_pend/';		
	$config['allowed_types'] = '*';		
	$config['overwrite'] = 'true';				
	$config['file_name'] = $id.'.'.$extensao;				
	$this->load->library('upload', $config);	
	...
	if($dadosArquivo[0]->arquivo_pendencias){
		...
		//unlink("teste.txt");
		//$this->config->base_url().'assets/cnd_imob_pend/'.$dadosArquivo[0]->arquivo_pendencias;exit;
		$a = @unlink($b.'/assets/cnd_est_pend/'.$dadosArquivo[0]->arquivo);
	}
 ...
 function atualizar_cnd_tratativa(){
	...
	$tipoCND = $this->input->get('tipoCND');
	...
	$statusCnpj = $this->input->get('statusCnpj');	
 ...
 function buscaCidade(){	
  ...
  function buscaCidadeByEstado(){	
	...
	$retorno ='';
	$result = $this->cnd_estadual_model->listarCidadeByEstado($idContratante,$id,'X',$ativo);
  ...
  function buscaImovelByCidade(){	
	...
	$ativo = $this->input->get('ativo');
	$retorno ='';
	$result = $this->loja_model->buscaLojaByCidadeFiltro($idContratante,$id,$ativo);
	if($result == 0){
		$retorno .="<option value='0'>Não Há Dados</option>";
	}else{	
		$retorno .="<option value=0>Escolha uma Loja</option>";
		foreach($result as $key => $iptu){ 			 
			$retorno .="<option value='".$iptu->id_loja."'>".$iptu->razao_social."</option>";		 
		}
 ...
 function buscaInscricaoByEstado(){	
	...
	$retorno ='';
	$result = $this->cnd_estadual_model->listarImovelByEstado($id);
 ...
 function buscarCnpjEst(){	
	$id = $this->input->get('id');
	
	$retorno ='';
	$result = $this->cnd_estadual_model->listarCnpjByLoja($id);
	...
	if($result == 0){
		...
		foreach($result as $key => $iptu){ 	
			...
			$retorno .="<option value=".$iptu->cnpj.">".$iptu->cnpj."</option>";
 ...
 function buscarInscrByCnpj(){	
	$id = $this->input->get('id');
	
	$retorno ='';
	$result = $this->cnd_estadual_model->listarInscrByCnpj($id);
	...
	if($result == 0){
		...
		foreach($result as $key => $iptu){ 	
			...
			$retorno .="<option value=".$iptu->inscricao.">".$iptu->inscricao."</option>";
