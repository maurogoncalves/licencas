...
Class Falencia_concordata_model extends CI_Model{

	function excluirFisicamente($id){
		$this->db->where('id', $id);
		$this->db->delete('falencia_concordata'); 
		return true;
	 } 
  ...
  function listarEstado($idContratante,$tipo){
	...
	$this -> db -> join('falencia_concordata','falencia_concordata.id_loja = loja.id');
	$this -> db -> where('falencia_concordata.id_contratante', $idContratante);
	if($tipo <> 'X'){
		$this -> db -> where('falencia_concordata.possui_cnd', $tipo);
	}
 ...
 function listarEsfera(){
  ...
  function listarCndTipoMun($id_contratante,$cidade,$tipo ){
   ...
   $this -> db -> select('*,falencia_concordata.ativo as status_insc,falencia_concordata.id as id_cnd,emitente.nome_fantasia as raz_soc,emitente.cpf_cnpj as cpf_cnpj_loja,loja.ins_cnd_mob,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('falencia_concordata'); 
   $this -> db -> join('loja','falencia_concordata.id_loja = loja.id');
   $this -> db -> join('imovel','imovel.id = loja.id_imovel');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> where('falencia_concordata.id_contratante', $id_contratante);
  ...
  function listarCndTipoId($id_contratante,$id,$tipo){
   ...
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> where('falencia_concordata.id_contratante', $id_contratante);
   $this -> db -> where('falencia_concordata.id', $id);
   $this -> db -> order_by('emitente.nome_fantasia');
   $query = $this -> db -> get();
   //print_r($this->db->last_query());exit;
   if($query -> num_rows() <> 0) {
 ...
 function listarCndTipoEst($id_contratante,$estado,$tipo ){
   ...
   $this -> db -> select('*,falencia_concordata.ativo as status_insc,falencia_concordata.id as id_cnd,emitente.razao_social as raz_soc,emitente.cpf_cnpj as cpf_cnpj_loja,loja.ins_cnd_mob,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('falencia_concordata'); 
   $this -> db -> join('loja','falencia_concordata.id_loja = loja.id');
   $this -> db -> join('imovel','imovel.id = loja.id_imovel');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
 ...
 function contaCndRegional($id_contratante,$ano,$tipo,$reg){
	 if($tipo == 0){
		 $sql = "SELECT count(`falencia_concordata`.`possui_cnd`) as total FROM (`falencia_concordata`) 
		left JOIN  `loja` ON `falencia_concordata`.`id_loja` = `loja`.`id`  JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
		JOIN `regional` ON `regional`.`id` = `loja`.`regional`  JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 WHERE `falencia_concordata`.`id_contratante` = ? and regional.id = ?
	  ";
	 $query = $this->db->query($sql, array($id_contratante,$reg));
		...
		left JOIN  `loja` ON `falencia_concordata`.`id_loja` = `loja`.`id`  JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
		JOIN `regional` ON `regional`.`id` = `loja`.`regional`  JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 WHERE `falencia_concordata`.`id_contratante` = ? and falencia_concordata.possui_cnd = ? and regional.id = ?
	  ";
	 $query = $this->db->query($sql, array($id_contratante,$tipo,$reg));	
		...
		JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 ...
		 and loja.id not in (select id_loja from falencia_concordata where id_contratante = ?)
	  ";
 ...
 $sql = "SELECT count(`falencia_concordata`.`possui_cnd`) as total, `possui_cnd` FROM (`falencia_concordata`) 
left JOIN 
`loja` ON `falencia_concordata`.`id_loja` = `loja`.`id` 
JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
JOIN `regional` ON `regional`.`id` = `loja`.`regional` 
JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
 WHERE `falencia_concordata`.`id_contratante` = ? GROUP BY `falencia_concordata`.`possui_cnd` 
";
 ...
 $this -> db -> select('*,loja.ins_cnd_mob,falencia_concordata.ativo as status_cnd,falencia_concordata.id as id_cnd,emitente.nome_fantasia as nome');
   ...
   if($tipo <> 'X'){
	...
	$this -> db -> order_by('falencia_concordata.possui_cnd');  
   }
 ...
 function listarTodasCidades(){
   ...
   $this -> db -> join('falencia_concordata','falencia_concordata.id_iptu = iptu.id');   
...
function listarTodasInscricoes($id_contratante){
  ...
  function listarLoja($id_contratante,$tipo){
   $this->db->distinct();
   $this -> db -> select('falencia_concordata.id,emitente.nome_fantasia as  nome');
   $this -> db -> from('loja');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> join('falencia_concordata','falencia_concordata.id_loja = loja.id');
   $this -> db -> where('falencia_concordata.id_contratante', $id_contratante);
...
function listarImovelByCidade($id,$tipo){
  ...
  public function add($detalhes = array()){
	...
	if($this->db->insert('falencia_concordata', $detalhes)) {
   ...
   if($query -> num_rows() <> 0){
	...
	$this->db->update('falencia_concordata', $dados); 
...
//print_r($this->db->last_query());exit;
 ...
 function listarInscricaoByLoja($id){
  ...
  $this -> db -> select('*,falencia_concordata.ativo as status_insc,falencia_concordata.id as id_cnd,loja.ins_cnd_mob as ins_cnd_mob,loja.id as id_loja,regional.descricao as desc_regional,bandeira.descricao_bandeira ');

   $this -> db -> from('falencia_concordata');
   
   $this -> db -> join('loja','falencia_concordata.id_loja = loja.id','left');
   ...
   $this -> db -> select('*,falencia_concordata.ativo as status_insc,falencia_concordata.id as id_cnd');
 ...
 function listarLojaByImovel($id_loja,$tipo){
   ...
   $this -> db -> select('*,falencia_concordata.ativo as status_insc,falencia_concordata.id as id_cnd,emitente.nome_fantasia as nome,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('falencia_concordata'); 
   $this -> db -> join('loja','falencia_concordata.id_loja = loja.id');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
  ...
  function listarImovelByUf($estado,$tipo){
   ...
   $this -> db -> where('falencia_concordata.possui_cnd', '3');  
