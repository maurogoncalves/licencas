...
Class Cnd_Mobiliaria_model extends CI_Model{
 ...
 function contarAreaFocal($id_contratante){
	...
	$sql = "SELECT  count(*) as total, area_focal.descricao_area_focal,area_focal.cor,area_focal.codigo FROM cnd_mob_tratativa  left join $tabela on cnd_mob_tratativa.id_cnd_mob = $tabela.id 	LEFT JOIN area_focal on cnd_mob_tratativa.id_area_focal = area_focal.codigo  WHERE `$tabela`.`id_contratante` = ? and status_chamado_sis_ext <> 2 and cnd_mob_tratativa.id_area_focal <> 1 and  cnd_mob_tratativa.modulo = $modulo group by area_focal.descricao_area_focal";
...
function listarTratativaMobById($idContratante,$id){
 ...
 function listarTratativaById($idContratante,$id){
	...
	$sql = "select cnd_mob_tratativa.id,
			...
			cnd_mob_tratativa.id_aplicacao_legado,
			cnd_mob_tratativa.link,
			cnd_mob_tratativa.login,
