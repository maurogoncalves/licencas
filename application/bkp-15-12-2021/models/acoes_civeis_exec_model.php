...
Class Acoes_civeis_exec_model extends CI_Model{

	function excluirFisicamente($id){
		$this->db->where('id', $id);
		$this->db->delete('acoes_civeis_exec'); 
		return true;
	 } 
  ...
  function listarEstado($idContratante,$tipo){
	...
	$this -> db -> join('acoes_civeis_exec','acoes_civeis_exec.id_loja = loja.id');
	$this -> db -> where('acoes_civeis_exec.id_contratante', $idContratante);
	if($tipo <> 'X'){
		$this -> db -> where('acoes_civeis_exec.possui_cnd', $tipo);
	}
 ...
 function addObsTrat($detalhes = array()){ 
	...
	$sql = "select cnd_mob_tratativa.id,cnd_mob_tratativa.pendencia,
		...
		where cnd_mob_tratativa.id_cnd_mob = ? and modulo = 2 order by cnd_mob_tratativa.id desc ";
...
print_r($this->db->last_query());exit;
 ...
 function listarEsfera(){
  ...
  function listarCndTipoMun($id_contratante,$cidade,$tipo ){
   ...
   $this -> db -> select('*,acoes_civeis_exec.ativo as status_insc,acoes_civeis_exec.id as id_cnd,emitente.nome_fantasia as raz_soc,emitente.cpf_cnpj as cpf_cnpj_loja,loja.ins_cnd_mob,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('acoes_civeis_exec'); 
   $this -> db -> join('loja','acoes_civeis_exec.id_loja = loja.id');
   $this -> db -> join('imovel','imovel.id = loja.id_imovel');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> where('acoes_civeis_exec.id_contratante', $id_contratante);
  ...
  function listarCndTipoId($id_contratante,$id,$tipo){
   ...
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> where('acoes_civeis_exec.id_contratante', $id_contratante);
   $this -> db -> where('acoes_civeis_exec.id', $id);
   $this -> db -> order_by('emitente.nome_fantasia');
   $query = $this -> db -> get();
   //print_r($this->db->last_query());exit;
   if($query -> num_rows() <> 0) {
 ...
 function listarCndTipoEst($id_contratante,$estado,$tipo ){
   ...
   $this -> db -> select('*,acoes_civeis_exec.ativo as status_insc,acoes_civeis_exec.id as id_cnd,emitente.razao_social as raz_soc,emitente.cpf_cnpj as cpf_cnpj_loja,loja.ins_cnd_mob,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('acoes_civeis_exec'); 
   $this -> db -> join('loja','acoes_civeis_exec.id_loja = loja.id');
   $this -> db -> join('imovel','imovel.id = loja.id_imovel');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
 ...
 function contaCndRegional($id_contratante,$ano,$tipo,$reg){
	 if($tipo == 0){
		 $sql = "SELECT count(`acoes_civeis_exec`.`possui_cnd`) as total FROM (`acoes_civeis_exec`) 
		left JOIN  `loja` ON `acoes_civeis_exec`.`id_loja` = `loja`.`id`  JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
		JOIN `regional` ON `regional`.`id` = `loja`.`regional`  JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 WHERE `acoes_civeis_exec`.`id_contratante` = ? and regional.id = ?
	  ";
	 $query = $this->db->query($sql, array($id_contratante,$reg));
		...
		left JOIN  `loja` ON `acoes_civeis_exec`.`id_loja` = `loja`.`id`  JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
		JOIN `regional` ON `regional`.`id` = `loja`.`regional`  JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 WHERE `acoes_civeis_exec`.`id_contratante` = ? and acoes_civeis_exec.possui_cnd = ? and regional.id = ?
	  ";
	 $query = $this->db->query($sql, array($id_contratante,$tipo,$reg));	
		...
		JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 ...
		 and loja.id not in (select id_loja from acoes_civeis_exec where id_contratante = ?)
	  ";
 ...
 $sql = "SELECT count(`acoes_civeis_exec`.`possui_cnd`) as total, `possui_cnd` FROM (`acoes_civeis_exec`) 
left JOIN 
`loja` ON `acoes_civeis_exec`.`id_loja` = `loja`.`id` 
JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
JOIN `regional` ON `regional`.`id` = `loja`.`regional` 
JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
 WHERE `acoes_civeis_exec`.`id_contratante` = ? GROUP BY `acoes_civeis_exec`.`possui_cnd` ";
 ...
 $this -> db -> select('*,loja.ins_cnd_mob,acoes_civeis_exec.ativo as status_cnd,acoes_civeis_exec.id as id_cnd,emitente.nome_fantasia as nome');
   ...
   if($tipo <> 'X'){
	...
	$this -> db -> order_by('acoes_civeis_exec.possui_cnd');  
   }
 ...
 function listarTodasCidades(){
   ...
   $this -> db -> join('acoes_civeis_exec','acoes_civeis_exec.id_iptu = iptu.id');   
...
function listarTodasInscricoes($id_contratante){
  ...
  function listarLoja($id_contratante,$tipo){
   $this->db->distinct();
   $this -> db -> select('acoes_civeis_exec.id,emitente.nome_fantasia as  nome');
   $this -> db -> from('loja');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> join('acoes_civeis_exec','acoes_civeis_exec.id_loja = loja.id');
   $this -> db -> where('acoes_civeis_exec.id_contratante', $id_contratante);
...
function listarImovelByCidade($id,$tipo){
  ...
  public function add($detalhes = array()){
	...
	if($this->db->insert('acoes_civeis_exec', $detalhes)) {
   ...
   if($query -> num_rows() <> 0){
	...
	$this->db->update('acoes_civeis_exec', $dados); 
...
//print_r($this->db->last_query());exit;
 ...
 function listarInscricaoByLoja($id){
  ...
  $this -> db -> select('*,acoes_civeis_exec.ativo as status_insc,acoes_civeis_exec.id as id_cnd,loja.ins_cnd_mob as ins_cnd_mob,loja.id as id_loja,regional.descricao as desc_regional,bandeira.descricao_bandeira ');
   ...
   $this -> db -> select('*,acoes_civeis_exec.ativo as status_insc,acoes_civeis_exec.id as id_cnd');
 ...
 function listarLojaByImovel($id_loja,$tipo){
   ...
   $this -> db -> select('*,acoes_civeis_exec.ativo as status_insc,acoes_civeis_exec.id as id_cnd,emitente.nome_fantasia as nome,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('acoes_civeis_exec'); 
   $this -> db -> join('loja','acoes_civeis_exec.id_loja = loja.id');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
  ...
  function listarImovelByUf($estado,$tipo){
   ...
   $this -> db -> where('acoes_civeis_exec.possui_cnd', '3');  
