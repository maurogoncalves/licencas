...
function totalInfracoesAnoStatusRegional($idContratante,$regional){
   ...
   $sql = "select count(*) as total,
			...
			WHEN infracoes.status = 2 THEN 'Vencida' else 'Não Definido'
			END as status from infracoes left join loja on loja.id = infracoes.id_loja  
			where infracoes.id_contratante =? and loja.regional =  ? group by infracoes.status";
  ...
  function totalInfracoesAnoStatus($idContratante){
   ...
   $sql = "select count(*) as total,
			CASE  WHEN infracoes.status = 1 THEN 'Em Análise'
			WHEN infracoes.status = 2 THEN 'Vencida' else 'Não Definido'
			END as status from infracoes left join loja on loja.id = infracoes.id_loja  where infracoes.id_contratante =?  group by infracoes.status";
...
function buscaIptuEstadoAno($anoAtual,$esseAnoMenosUm,$esseAnoMenosDois,$esseAnoMenosTres,$esseAnoMenosQuatro,$idContratante,$estados){      
	$sql = "SELECT estado,				
				COALESCE(round(sum(replace(REPLACE(ip.valor, '.', ''),',','.')),1), 0) as  primeiro,				
				COALESCE(round(sum(replace(REPLACE(ip1.valor, '.', ''),',','.')),1), 0) as segundo,				
				COALESCE(round(sum(replace(REPLACE(ip2.valor, '.', ''),',','.')),1), 0) as terceiro,				
				COALESCE(round(sum(replace(REPLACE(ip3.valor, '.', ''),',','.')),1), 0) as quarto,				
				COALESCE(round(sum(replace(REPLACE(ip4.valor, '.', ''),',','.')),1), 0) as quinto				
				FROM (`imovel`) 
				left JOIN iptu  as ip ON ip.`id_imovel` = `imovel`.`id` 				and ip.ano_ref = ?				
				left JOIN iptu as ip1 ON ip1.`id_imovel` = `imovel`.`id` 				and ip1.ano_ref = ?				
				left JOIN iptu as ip2 ON ip2.`id_imovel` = `imovel`.`id` 				and ip2.ano_ref = ?				
				left JOIN iptu as ip3 ON ip3.`id_imovel` = `imovel`.`id` 				and ip3.ano_ref = ?				
				left JOIN iptu as ip4 ON ip4.`id_imovel` = `imovel`.`id` 				and ip4.ano_ref = ?				
				WHERE `id_contratante` = ? 				and imovel.estado in ($estados)				
				group by estado				order by primeiro desc			";   
	$query = $this->db->query($sql, array($anoAtual,$esseAnoMenosUm,$esseAnoMenosDois,$esseAnoMenosTres,$esseAnoMenosQuatro,$idContratante,$estados));     
	if($query -> num_rows() <> 0){     
		return $query->result();   
	}else{    
		return false;   
...
function listarImovelCsv($id_contratante){  
	...
	$sql = "SELECT
		...
		0	as area_total_iptu,
		0	as area_construida_iptu		
		FROM (`imovel`) 
...
function contaVencerTrinta($idContratante, $iniTrintaVencer,$fimTrintaVencer){  
	 ...
	 if($iniTrintaVencer == 1){
		$sql = "select count(*) as total, l.estado from ".$tabela." c  join loja l on l.id = c.id_loja where c.id_contratante = ? and c.data_vencto > ?  and c.ativo = 0 group by l.estado";
		$query = $this->db->query($sql, array($idContratante, $fimTrintaVencer));
	}else{
		
		 if($iniTrintaVencer <> 0){
			$sql = "select count(*) as total, l.estado from ".$tabela." c  join loja l on l.id = c.id_loja where c.id_contratante = ? and c.data_vencto between ? and ? and c.ativo = 0 group by l.estado";
			$query = $this->db->query($sql, array($idContratante, $iniTrintaVencer,$fimTrintaVencer));
		}else{
			$sql = "select count(*) as total, l.estado from ".$tabela." c  join loja l on l.id = c.id_loja where c.id_contratante = ? and c.data_vencto < ? and c.ativo = 0 group by l.estado";
			$query = $this->db->query($sql, array($idContratante, $fimTrintaVencer));
		}
