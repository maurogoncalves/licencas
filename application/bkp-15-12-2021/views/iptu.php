...
class Iptu extends CI_Controller {
 ...
 function cadastrar(){  if(!$this->session->userdata('logged_in')){		
	redirect('login', 'refresh');  }			
	$anos = anos();		
	$session_data = $this->session->userdata('logged_in');		
...
redirect('/iptu/listar', 'refresh');		 
 ...
 function cadastrar_iptu(){    if(!$this->session->userdata('logged_in')){		redirect('login', 'refresh');  }
	$session_data = $this->session->userdata('logged_in');
	$idContratante = $session_data['id_contratante'];
	$id_imovel = $this->input->post('id_imovel');			$inscricao = $this->input->post('inscricao');			$valor = $this->input->post('valor');	
	$areaTotal = $this->input->post('areaTotal');	
	$areaConstruida = $this->input->post('areaConstruida');	
	$nome = $this->input->post('nome');		$status = $this->input->post('status');			$informacoes = $this->input->post('informacoes');			$observacoes = $this->input->post('observacoes');			$ano_ref = $this->input->post('ano_ref');		
	$dados = array(
					'id_imovel' => $id_imovel,
					'inscricao' => $inscricao,										'valor' => $valor,					'area_total' => $areaTotal,					
					'area_construida' => $areaConstruida,
					'nome_proprietario' => $nome,
					'status_prefeitura' => $status,
					'info_inclusas' => $informacoes,
					'observacoes' => $observacoes,										'ano_ref' => $ano_ref,
 ...
 $id = $this->input->get('id');	$dados = array('ativo' => 1);	
 if($this->iptu_model->atualizar($dados,$id)) { 	
 ...
 function atualizar_iptu(){	
	...
	$dadosAtuais = $this->iptu_model->listarImovelByIdIptu($id);	
	...
	if($dadosAtuais[0]->id_im <> $id_imovel){		
	...
	$dadosImovel = $this->iptu_model->listarImovelByImovel($dadosAtuais[0]->id_im);	
	$this->session->set_userdata('cidadeIpBD', $dadosImovel[0]->cidade);
	$this->session->set_userdata('estadoIpBD', $dadosImovel[0]->estado);
	$this->session->set_userdata('idImIpBD', $dadosImovel[0]->id_imovel);
...
function export_est(){	    
 ...
 function export_todos(){	   
  ...
  function export(){	   
	...
	$this->csv($result);	
			...
			function buscaCidade(){		$id = $this->input->get('estado');		$retorno ='';	$result = $this->iptu_model->listarCidadeByEstado($id);			if($result == 0){		$retorno .="<option value='X'>Não Há Dados</option>";	}else{			$retorno .="<option value='X'>Escolha uma Cidade</option>";		foreach($result as $key => $iptu){ 			 			$retorno .="<option value='".$iptu->cidade."'>".$iptu->cidade."</option>";		 		}		}	echo json_encode($retorno);	 }  function buscaTodasCidades(){			$retorno ='';	$result = $this->iptu_model->listarTodasCidades();			if($result == 0){		$retorno .="<option value='X'>Não Há Dados</option>";	}else{			$retorno .="<option value='X'>Escolha uma Cidade</option>";		foreach($result as $key => $iptu){ 			 			$retorno .="<option value='".$iptu->cidade."'>".$iptu->cidade."</option>";		 		}		}	echo json_encode($retorno);	 } 
			...
			function buscaImovelByEstado(){		
				...
				$result = $this->iptu_model->listarImovelByEstado($id);		
					if($result == 0){		
						$retorno .="<option value='X'>Não Há Dados</option>";	
					}else{			
						$retorno .="<option value='X'>Escolha um Imóvel</option>";		
							foreach($result as $key => $iptu){ 			 			
			...
			function buscaImovel(){		$id = $this->input->get('id');		$retorno ='';	$result = $this->iptu_model->listarImovelByCidade($id);		if($result == 0){		$retorno .="<option value='X'>Não Há Dados</option>";	}else{			$retorno .="<option value='X'>Escolha um Imóvel</option>";		foreach($result as $key => $iptu){ 			 			$retorno .="<option value=".$iptu->id.">".$iptu->nome."</option>";		 		}		}	echo json_encode($retorno);	 }  
 ...
 function listar(){	  
	...
	$data['estados'] = $this->iptu_model->listarEstado();		
	
	//$result = $this->iptu_model->listarIptu($idContratante,numRegister4PagePaginate(), $page,$ano);		
	...
	$cidadeBD = $this->session->userdata('cidadeIpBD');
	if(empty($cidadeBD)){
		$data['cidadeBD'] = 0;
	}else{
		$data['cidadeBD'] =$this->session->userdata('cidadeIpBD');
	}
	
	$estadoBD = $this->session->userdata('estadoIpBD');
	if(empty($estadoBD)){
		$data['estadoBD'] = 0;
	}else{
		$data['estadoBD'] = $this->session->userdata('estadoIpBD');
	}
	
	$idImIpBD = $this->session->userdata('idImIpBD');
	if(empty($idImIpBD)){
		$data['idImIpBD'] = 0;
	}else{
		$data['idImIpBD'] =$this->session->userdata('idImIpBD');
	}
	
	if(empty($_SESSION["status_iptu"])){
		$data['status_iptu'] = 0;
	}else{
		$data['status_iptu'] =$_SESSION["status_iptu"];
	}
