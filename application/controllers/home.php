...
class Home extends CI_Controller {
 ...
 function __construct()
 {
  ...
  function report(){	
	...
	function mapa(){
	  ...
	  $data['licencasEstados'] =  $this->loja_model->contarCndByStatus($idContratante,'BR',1);		
	...
	function mapa_taxas(){
	 ...
	 foreach($estados as $est){		
		...
		$dadosEstado =  $this->loja_model->contarCndByEst($idContratante,$est->uf,2);	
	...
	$data['licencasEstados'] =  $this->loja_model->contarCndByMesVencto($idContratante,'BR',2);	
	$data['todasCnds'] =  $this->loja_model->contarCndByEst($idContratante,$estado,2);	
	
	$this->load->view('header_pages_view',$data);
    $this->load->view('dados_agrupados_mapa_taxa', $data);
	$this->load->view('footer_pages_view');
 ...
 function grafico_tipo(){
	...
	$pendencias = $this->cnd_mobiliaria_model->contarTipoLicencas(); 
	...
	$taxas = $this->cnd_mobiliaria_model->contarTipoTaxa(); 
	...
	if($taxas == 0){
		...
		$coresTaxas ='';
		foreach($taxas as $pen){
			$labelsTaxas .= '"'.$pen->descricao.'",';
			if($pen->cor){
				$coresTaxas .= '"'.$pen->cor.'",';
			}else{
				$coresTaxas .= '"#fefefe",';
		...
		$data['labelsTaxas'] = $labelsTaxas;
		$data['taxas'] = $taxas;
		$data['coresTaxas'] =$coresTaxas;
	}
