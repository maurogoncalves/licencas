...
class Loja extends CI_Controller {
 ...
 if(!$_SESSION['loginTeste']) {
	...
	if($tipo_licenca == 0){
		echo "<script>alert('Escolha um tipo de licen�a'); window.history.go(-1);</script>";
		exit;
	}
 ...
 function buscaEmitente(){	
	...
	if($isArray == 0){
		$retorno .="<option value='0'>N�o H� Dados</option>";
	}else{
  ...
  function buscaLojaByCidade(){	
	...
	if($isArray == 0){
		...
		foreach($result as $imovel){ 	
			...
			$retorno .="<option value='".$imovel->id."'>".$imovel->nome_loja."</option>";
 ...
 function buscaCnpj(){	
	...
	$retorno ='';
	$result = $this->loja_model->buscaCnpj($idContratante,$imovel,$cidade,$estado,$ativo);
 ...
 function buscaCnd(){	
	...
	$retorno ='';
	$result = $this->loja_model->buscaCnd($idContratante,$cnpj);
	...
	if($isArray == 0){
		...
		foreach($result as $imovel){ 	
			...
			$retorno .="<option value='".$imovel->ins_cnd_mob."'>".$imovel->ins_cnd_mob."</option>";
...
$isArray =  is_array($result) ? '1' : '0';
	...
	if($isArray == 0){
		$retorno .="<tr>";
		$retorno .="<td class='hidden-phone' colspan='5'> N�o H� Dados </td>";
		$retorno .="</tr>";
	}else{
		foreach($result as $imovel){ 
 ...
 function buscaCidadeByEstado(){	
	...
	$retorno ='';
	$result = $this->loja_model->buscaCidade($idContratante,$id,$ativo);
 ...
 function buscaCidadeImByEstado(){	
	...
	if($isArray == 0){
		...
		foreach($result as $key => $imovel){ 	
			...
			$retorno .="<option value='".$imovel->nome."'>".$imovel->nome."</option>";
 ...
 function excluir(){
	...
	if($podeExcluir[0]['total'] == 1){
		if($this->loja_model->excluirFisicamente($id)) {
			$_SESSION['mensagemLoja'] =  CADASTRO_APAGADO;
		}else{	
			$_SESSION['mensagemLoja'] =  ERRO;
		}
 ...
 function cadastrar_loja(){
	...
	$nome_loja = $this->input->post('nome_loja');	
	...
	if($op =='0'){
		$this->session->set_flashdata('messagemLoja', 'Loja cadastrada');
		...
		$this->loja_model->addCnd($dadosCnd);
	}else{
		$this->session->set_flashdata('messagemLoja', 'Loja alterada');
		$this->loja_model->atualizar($dados,$id);
	}
 ...
 function atualizar_loja(){
	...
	$conjunta = $this->input->post('conjunta');	
	...
	if($dadosAtuais[0]->id_emitente <> $emitente){
		$dadosAlterados .= ' - Raz�o Social: '.$dadosEmitente[0]->razao_social;	
	}
	
	if($dadosAtuais[0]->id_imovel <> $id_imovel){
		$dadosAlterados .= ' - Im�vel: '.$dadosImovel[0]->nome;	
	}
	...
	if($dadosAtuais[0]->bandeira <> $bandeira){		if(empty($dadosBandeira[0]->descricao_bandeira)){			$dadosAlterados .= ' - Bandeira: '.utf8_decode('Sem Descri��o');		
	...
	if($dadosAtuais[0]->numero <> $numero){
		$dadosAlterados .= ' - N�mero: '.$dadosAtuais[0]->numero;	
	}
 ...
 function csv($result){
	 ...
	 $file="lojas.xls";
		...
		if($isArray == 0){
			$test="
			<tr>
			<td>N�o H� Dados para exibi&ccedil;&atilde;o</td>		
			</tr>
			";
		}else{			
			  ...
			  $isArrayLog =  is_array($dadosLog) ? '1' : '0';
				...
				$dadosCnd = $this->loja_model->buscaInscricaoCSV($emitente->id_loja);	
				if(!empty($dadosCnd[0]->inscricao)){
					$inscricaoEst = $dadosCnd[0]->inscricao;
				}else{
					$inscricaoEst = '-';
				}
				...
				$test .= "<td>"."'".utf8_decode($inscricaoEst)."'"."</td>";
...
function listar_status(){	
		...
		if($isArrayCndEst <> 0) {			
			foreach($arrayCndEst as $cnd_est){
				...
				if($cnd_est['possui_cnd'] == 1){
					...
					$arrCNDEst[$j]['possui_cnd_est'] ='N�o';
				}else{
 ...
 function listar(){	
	...
	if(empty($_POST)){
		...
		$data['tabela'] = $tabela;
		$data['emitentes']  = $this->loja_model->listarTodasLoja($idContratante);
		...
		if($idImovelListar == '0'){	
			if($municipioListar <> '0'){
				//print'aqui';exit;
				$data['emitentes']  = $this->loja_model->buscaLojaByCidade($idContratante,$municipioListar,$status,$tabela);
			}else if($estadoListar <> '0'){
				//print'aqui1';exit;
				$data['emitentes']  = $this->loja_model->buscaLojaByEstado($idContratante,$estadoListar,$status,$tabela);				
			}else{
				//print'aqui2';exit;
				$data['emitentes']  = $this->loja_model->buscaLojaById($idContratante,$idImovelListar,$status,$tabela);
