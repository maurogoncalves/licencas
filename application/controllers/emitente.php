...
class Emitente extends CI_Controller {
 ...
 function cadastrar_emitente(){
	...
	if($cpfCnpj){
		...
		if($temCnpj[0]->total <> 0){
			echo "<script>alert('CPF/CNPJ ja existe'); window.history.go(-1);</script>";
			exit;
		}
	}
	...
	$idEmitente = $this->emitente_model->add($dados);
 ...
 function buscarCnpjEmitente(){	
	$cnpjMatriz = $this->input->get('cnpjMatriz');
	$session_data = $_SESSION['loginTeste'];
	$idContratante = $session_data['id_contratante'];
	$result = $this->emitente_model->listarCnpjEmitenteMatriz($idContratante,$cnpjMatriz);
	
	$retorno = '';
	if($result == 0){
		$retorno .=" <option value='0'>Sem Emitentes</option>";
	}else{	
		$retorno .="<option value='0'>Escolha um emitente</option>";		
		foreach($result as $key => $emitente){ 	
			$retorno .="<option value='".$emitente->id."'>".$emitente->nome_fantasia." + ".$emitente->cpf_cnpj."</option>";
		}	
	}
 ...
 function listar(){	
	...
	if($_SESSION['loginTeste']) {
	 ...
	 if(empty($session_data['visitante'])){
		...
		$result = $this->emitente_model->listarEmitente($idContratante);
		$total = $this->emitente_model->somarTodos($idContratante);
		$data['paginacao'] = createPaginate('emitente', $total[0]->total) ;
