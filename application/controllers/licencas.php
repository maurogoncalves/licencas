...
EOD;
 ...
 function cadastrar_cnd_estadual(){
	...
	//print_r($this->db->last_query());exit;
	redirect('licencas/visao_interna_cnd_est?id='.$id, 'refresh');
 ...
 function limpar_filtro(){
	...
	redirect('licencas/lista', 'refresh');
 }	 
 ...
 function lista(){
	...
	$data['tipo_cnd'] = 'Licenças';
	$data['nome_cnd'] = 'Licenças - Ativas';	
	if(empty($_POST)){
 ...
 function visao_interna_cnd_imob(){
   ...
   if(empty($_SESSION['idTratativa'])){
	...
	$data['periodoRenovacao'] = $this->cnd_imobiliaria_model->listarPeriodoRenovacao();
	$data['periodoTaxa'] = $this->cnd_imobiliaria_model->listarPeriodoTaxa();
...
if($modulo == 1){
	redirect('licencas/visao_interna_cnd_mob?id='.$id_cnd_mob, 'refresh');		 
}elseif($modulo == 2){
	redirect('licencas/visao_interna_cnd_imob?id='.$id_cnd_mob, 'refresh');		 
}elseif($modulo == 3){
	redirect('licencas/visao_interna_cnd_est?id='.$id_cnd_mob, 'refresh');		 
}else{
	redirect('licencas/visao_interna_cnd_fed?id='.$id_cnd_mob, 'refresh');		 
}	
...
function apagarArquivosEst(){
	...
	redirect('/licencas/visao_interna_cnd_mob?id='.$idCnd);
...
function listaArquivosEst(){
	...
	if($isArrayLog == 1) {
		foreach($dados as $dado){
			...
			if(!empty($dado->arquivo)){
				...
				$excluir = "<a href=".$base."/index.php/licencas/apagarArquivosEst?id=".$dado->id_arquivo."&idCnd=".$idArr[1]." >  <i class='fa fa-trash' aria-hidden='true'></i></a>";		
				if($dado->observacao){
 ...
 function enviar(){		
	...
	if (!$this->upload->do_upload($field_name)){			
		...
		redirect('/licencas/visao_interna_cnd_mob?id='.$id);
...
function subir_arquivo_chamado(){		
	...
	redirect('/licencas/listar_chamados');
...
function visao_interna_cnd_est(){
 ...
 function visao_interna_cnd_mob(){
   ...
   if(empty($_SESSION['idTratativa'])){
	...
	// if($data['imovel'][0]->total == 0){
		// redirect('licencas/cnd_lista', 'refresh');
	// }
	$data['valorTotalPendencia'] = $this->cnd_mobiliaria_model->listarInscricaoByLoja($id);	
	$data['id_cnd'] = $id;
	$data['tratativas'] =  $this->cnd_mobiliaria_model->listarTodasTratativas($idContratante,$id,$_SESSION['loginTeste']['tipo_cnd'],1);
	$data['taxas'] =  $this->cnd_mobiliaria_model->listarTodasTratativas($idContratante,$id,$_SESSION['loginTeste']['tipo_cnd'],2);
	$data['esferas'] = $this->cnd_mobiliaria_model->listarEsfera();
	$data['etapas'] = $this->cnd_mobiliaria_model->listarEtapa();
	$data['area_focal'] = $this->cnd_mobiliaria_model->listarAreaFocal();
	$data['natureza_raiz'] = $this->cnd_mobiliaria_model->listarNaturezaRaiz();
	$data['tipo_aplicacao'] = $this->cnd_mobiliaria_model->listarTipoAplicacao();
	...
	$data['periodoRenovacao'] = $this->cnd_imobiliaria_model->listarPeriodoRenovacao();
	$data['periodoTaxa'] = $this->cnd_imobiliaria_model->listarPeriodoTaxa();
	$data['tipoTaxa'] = $this->cnd_imobiliaria_model->listarTipoTaxa();
 ...
 function listarTratativaMobById(){
	...
	$data['senha'] = $dados[0]->senha;
	$data['regra_taxa'] = $dados[0]->regra_taxa;
	$data['competencia'] = $dados[0]->competencia;
	$data['org_resp'] = $dados[0]->org_resp;
	$data['modo_emissao'] = $dados[0]->modo_emissao;
	$data['periodo_taxas'] = $dados[0]->periodo_taxas;
	$data['periodo_renovacao'] = $dados[0]->periodo_renovacao;
	$data['responsavel_pagamento'] = $dados[0]->responsavel_pagamento;
	$data['mes_pagamento'] = $dados[0]->mes_pagamento;
	$data['observacao'] = $dados[0]->observacao;
	$data['tipo_taxa'] = $dados[0]->tipo_taxa;
	$data['data_vencto'] = $dados[0]->data_vencto;
...
function export_tratativas_ext_fed(){	
 ...
 function atualizar_cnd_imob_tratativa_unica(){
	...
	$data_informe_pendencia 	= $this->input->post('data_criacao');
	$data_inclusao_sis_ext 	= $this->input->post('data_vencto');
	//$data_envio 	= $this->input->post('data_expiracao');
	$data_envio 	= '1111-11-11';
	...
	$id_aplicacao_legado = $this->input->post('id_aplicacao_legado');
	$link = $this->input->post('link');
	$login = $this->input->post('login');
	$senha = $this->input->post('senha');
	$regra_taxa = $this->input->post('regra_taxa');
	$competencia = $this->input->post('competencia');
	$org_resp = $this->input->post('org_resp');
	...
	if($modulo == 1){	
		redirect('/licencas/visao_interna_cnd_mob?id='.$id_cnd);
	}elseif($modulo == 2){	
		redirect('/licencas/visao_interna_cnd_imob?id='.$id_cnd);
	}
...
function atualizar_cnd_est_tratativa_unica(){
	...
	redirect('/licencas/visao_interna_cnd_est?id='.$id_cnd);
 ...
 function cadastrar_cnd_fed(){
	...
	redirect('/licencas/visao_interna_cnd_fed?id='.$id);
 ...
 function atualizar_cnd_fed_tratativa_unica(){
	...
	redirect('/licencas/visao_interna_cnd_fed?id='.$id_cnd);
...
function atualizar_cnd_mob_tratativa_unica(){
	...
	$id_sis_ext 	= $this->input->post('id_sis_ext');
	$data_inclusao_sis_ext 	= $this->input->post('data_inclusao_sis_ext');
	$prazo_solucao_sis_ext 	= $this->input->post('data_emissao');
	$data_encerramento_sis_ext 	= $this->input->post('data_encerramento_sis_ext');
	$status_chamado_sis_ext 	= $this->input->post('status_chamado_sis_ext');
	$id_sla = $this->input->post('id_sla');
	...
	$responsavel_pagamento = $this->input->post('responsavel_pagamento');
	...
	$org_resp = $this->input->post('org_resp');
	$observacao = $this->input->post('observacao');
	$periodo_taxas = $this->input->post('periodo_taxas');
	$modo_emissao = $this->input->post('modo_emissao');
	...
	if(empty($data_vencto)){
		$dataVencto = '1111-11-11';	
	}else{
		
		if($data_vencto == '00/00/0000'){
			$dataVencto = '1111-11-11';	
		}else{
			$dataVenctoArr = explode("/",$data_vencto);	
			$dataVencto = $dataVenctoArr[2].'-'.$dataVenctoArr[1].'-'.$dataVenctoArr[0];
		}
	...
	$dados = array(
		...
		'periodo_renovacao'=>$periodo_renovacao,
		'responsavel_pagamento'=>$responsavel_pagamento,
		'mes_pagamento'=>$mes_pagamento,
		'senha'=>$senha,
		'regra_taxa'=>$regra_taxa,
		'competencia'=>$competencia,
		'org_resp'=>$org_resp,
		'data_vencto'=>$dataVencto,
		'periodo_taxas'=>$periodo_taxas,		
		'observacao'=>$observacao,	
	...
	public function limpar()	{		
		redirect('licencas/listar_pendencia', 'refresh');
	}	
