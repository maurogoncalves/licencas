...
Class Loja_model extends CI_Model{
 ...
 function buscaCNDEstById($id_loja){
  ...
  function listarTodasLoja($id_contratante,$cidade,$estado ){
   $sql = "SELECT `loja`.*, `emitente`.`nome_fantasia` as razao_social, `emitente`.`cpf_cnpj`, `tipo_emitente`.`descricao`, `emitente`.`nome_resp`, `cnd_mobiliaria`.`possui_cnd` as cnd, `cnd_mobiliaria`.`id` as id_cnd, `bandeira`.`descricao_bandeira` as bandeira, `cnd_mobiliaria`.`data_vencto`, `cnd_mobiliaria`.`data_pendencias` 
   FROM (`loja`) JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` JOIN `tipo_emitente` ON `tipo_emitente`.`id` = `emitente`.`tipo_emitente`
  JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
  LEFT JOIN `cnd_mobiliaria` ON `cnd_mobiliaria`.`id_loja` = `loja`.`id`
  WHERE `loja`.`id_contratante` = ?
  AND (`emitente`.`tipo_emitente` = 2  OR `emitente`.`tipo_emitente` = 1 OR `emitente`.`tipo_emitente` = 7) 
...
$sql = "select loja.id as id_loja,emitente.nome_fantasia,emitente.cpf_cnpj,acomp_cnd.id as id_acomp, emitente.id as id_emitente, emitente.nome_fantasia,loja.cidade,loja.ins_cnd_mob, 
 ...
 function listarEstado($id_contratante){
  ...
  function listarCidade($id_contratante){
   ...
   $this -> db -> select('loja.*,emitente.nome_fantasia as razao_social,emitente.cpf_cnpj,tipo_emitente.descricao,emitente.nome_resp,possui_cnd as cnd, cnd_mobiliaria.id as id_cnd,bandeira.descricao_bandeira as bandeira,cnd_mobiliaria.data_vencto,cnd_mobiliaria.data_pendencias');
   $this -> db -> from('loja');
...
$sql = "SELECT `emitente`.`id`, `emitente`.`nome_fantasia`  FROM (`emitente`) JOIN `tipo_emitente`  ON `tipo_emitente`.`id` = `emitente`.`tipo_emitente` 
 ...
 function buscaLojaByEstado($idContratante,$id,$status){
 ...
 function buscaLojaById($idContratante,$id,$status){
	 
	if($id <> 0){ 
		$sql = "SELECT `loja`.*, loja.id as id_loja,cnd_mobiliaria.id as id_cnd, `emitente`.`nome_fantasia` as razao_social,emitente.cpf_cnpj,tipo_emitente.descricao as tipo_emitente,bandeira.descricao_bandeira,possui_cnd as cnd FROM (`loja`) 
		LEFT JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
		LEFT JOIN `tipo_emitente` ON `tipo_emitente`.`id` = `emitente`.`tipo_emitente` 
		LEFT JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira` 
		LEFT JOIN cnd_mobiliaria ON cnd_mobiliaria.id_loja = loja.id
 ...
 function buscaLojaByCidade($idContratante,$id,$status){
