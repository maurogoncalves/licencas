...
Class Cnd_Imobiliaria_model extends CI_Model{
 ...
 function listarIptuCsv($id_contratante,$possuiCnd){
  ...
  function listarIptuCsvByCidade($id_contratante,$possuiCnd){
 ...
 function listarIptuCsvByEstado($id_contratante,$possuiCnd){
 ...
 function listarCidadeByEstado($id,$tipo){
...
function listarTodasInscricoes($id_contratante){
  ...
  function listarImovel($id_contratante,$tipo){
   $this->db->distinct();
 ...
 function listarImovelByImovel($id_imovel,$tipo){
  ...
  function listarImovelByUf($estado,$tipo){
   ...
   if($query -> num_rows() <> 0){
    ...
    function contaCnd($idContratante,$ano){
	 
	$sql = "SELECT count(`cnd_imobiliaria`.`possui_cnd`) as total, `possui_cnd` FROM (`imovel`) 
	...
	WHERE iptu.`id` not in (select id_iptu from cnd_imobiliaria ) and ano_ref = ?
	 AND `imovel`.`id_contratante` = ? ";

	$query = $this->db->query($sql, array($ano,$idContratante,$ano,$idContratante));
   ...
   function contaSituacao($idContratante,$ano,$tipo,$situacao){
	   
	 if($tipo == 4){
		 $sql = "SELECT count(*) as total
		FROM 
		`iptu` 
		JOIN `imovel` ON `imovel`.`id` = `iptu`.`id_imovel` 
		left join tipo_situacao on tipo_situacao.id = imovel.situacao
		WHERE   iptu.ano_ref = ? and imovel.id_contratante = ? and
		iptu.id not in (select id_iptu from cnd_imobiliaria where cnd_imobiliaria.id_contratante = ?)
		 ";
		 $query = $this->db->query($sql, array($ano,$idContratante,$idContratante));
	 }else{
		 $sql = "SELECT count(*) as total
		FROM (`cnd_imobiliaria`) 
		JOIN `iptu` ON `cnd_imobiliaria`.`id_iptu` = `iptu`.`id` 
		JOIN `imovel` ON `imovel`.`id` = `iptu`.`id_imovel` 
		left join tipo_situacao on tipo_situacao.id = imovel.situacao
		WHERE `cnd_imobiliaria`.`id_contratante` = ?
		 AND `cnd_imobiliaria`.`possui_cnd` = ?
		  and iptu.ano_ref = ? and tipo_situacao.id = ?";
		 $query = $this->db->query($sql, array($idContratante,$tipo,$ano,$situacao));		 
	 }	 
