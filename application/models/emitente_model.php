...
Class Emitente_model extends CI_Model{
  function listarEmitente($id_contratante){
   //$this->db->limit( $_limit, $_start ); 	
   $this -> db -> select('emitente.id,emitente.nome_fantasia,emitente.razao_social,emitente.nome_resp,tipo_emitente.descricao,emitente.ativo,tipo_emitente.cor as cor,emitente.cpf_cnpj as cpf_cnpj');
 ...
 function listarCnpjMatriz(){
   $this -> db -> select('*');
   $this -> db -> from('cnpj_matriz');
   $query = $this -> db -> get();
   //print_r($this->db->last_query());exit;
   if($query -> num_rows() <> 0){
     return $query->result();
 ...
 function listarCnpjEmitenteMatriz($id_contratante,$cnpj_matriz){
   $this -> db -> select('emitente.id,emitente.nome_fantasia,emitente.cpf_cnpj');
   $this -> db -> from('emitente');
   $this -> db -> where('id_contratante', $id_contratante);
   $this -> db -> where('emitente.cnpj_matriz', $cnpj_matriz);  
   $this -> db -> order_by('emitente.nome_fantasia');	
   $query = $this -> db -> get();
 ...
 public function add($detalhes = array()){
	...
	if($this->db->insert('emitente', $detalhes)) {		return $id = $this->db->insert_id();
	}
