...
Class Tributos_mobiliarios_model extends CI_Model{

	function excluirFisicamente($id){
		$this->db->where('id', $id);
		$this->db->delete('tributos_mobiliarios'); 
		return true;
	 } 
  ...
  function listarEstado($idContratante,$tipo){
	...
	$this -> db -> join('tributos_mobiliarios','tributos_mobiliarios.id_loja = loja.id');
	$this -> db -> where('tributos_mobiliarios.id_contratante', $idContratante);
	if($tipo <> 'X'){
		$this -> db -> where('tributos_mobiliarios.possui_cnd', $tipo);
	}
 ...
 function addObsTrat($detalhes = array()){ 
	...
	$sql = "select cnd_mob_tratativa.id,cnd_mob_tratativa.pendencia,
		...
		where cnd_mob_tratativa.id_cnd_mob = ?  and modulo = 4 order by cnd_mob_tratativa.id desc ";
 ...
 function listarEsfera(){
  ...
  function listarCndTipoMun($id_contratante,$cidade,$tipo ){
   ...
   $this -> db -> select('*,tributos_mobiliarios.ativo as status_insc,tributos_mobiliarios.id as id_cnd,emitente.nome_fantasia as raz_soc,emitente.cpf_cnpj as cpf_cnpj_loja,loja.ins_cnd_mob,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('tributos_mobiliarios'); 
   $this -> db -> join('loja','tributos_mobiliarios.id_loja = loja.id');
   $this -> db -> join('imovel','imovel.id = loja.id_imovel');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> where('tributos_mobiliarios.id_contratante', $id_contratante);
  ...
  function listarCndTipoId($id_contratante,$id,$tipo){
   ...
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> where('tributos_mobiliarios.id_contratante', $id_contratante);
   $this -> db -> where('tributos_mobiliarios.id', $id);
   $this -> db -> order_by('emitente.nome_fantasia');
   $query = $this -> db -> get();
   //print_r($this->db->last_query());exit;
   if($query -> num_rows() <> 0) {
 ...
 function listarCndTipoEst($id_contratante,$estado,$tipo ){
   ...
   $this -> db -> select('*,tributos_mobiliarios.ativo as status_insc,tributos_mobiliarios.id as id_cnd,emitente.razao_social as raz_soc,emitente.cpf_cnpj as cpf_cnpj_loja,loja.ins_cnd_mob,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('tributos_mobiliarios'); 
   $this -> db -> join('loja','tributos_mobiliarios.id_loja = loja.id');
   $this -> db -> join('imovel','imovel.id = loja.id_imovel');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
 ...
 function contaCndRegional($id_contratante,$ano,$tipo,$reg){
	 if($tipo == 0){
		 $sql = "SELECT count(`tributos_mobiliarios`.`possui_cnd`) as total FROM (`tributos_mobiliarios`) 
		left JOIN  `loja` ON `tributos_mobiliarios`.`id_loja` = `loja`.`id`  JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
		JOIN `regional` ON `regional`.`id` = `loja`.`regional`  JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 WHERE `tributos_mobiliarios`.`id_contratante` = ? and regional.id = ?
	  ";
	 $query = $this->db->query($sql, array($id_contratante,$reg));
		...
		left JOIN  `loja` ON `tributos_mobiliarios`.`id_loja` = `loja`.`id`  JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
		JOIN `regional` ON `regional`.`id` = `loja`.`regional`  JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 WHERE `tributos_mobiliarios`.`id_contratante` = ? and tributos_mobiliarios.possui_cnd = ? and regional.id = ?
	  ";
	 $query = $this->db->query($sql, array($id_contratante,$tipo,$reg));	
		...
		JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
		 ...
		 and loja.id not in (select id_loja from tributos_mobiliarios where id_contratante = ?)
	  ";
 ...
 $sql = "SELECT count(`tributos_mobiliarios`.`possui_cnd`) as total, `possui_cnd` FROM (`tributos_mobiliarios`) 
left JOIN 
`loja` ON `tributos_mobiliarios`.`id_loja` = `loja`.`id` 
JOIN `emitente` ON `emitente`.`id` = `loja`.`id_emitente` 
JOIN `regional` ON `regional`.`id` = `loja`.`regional` 
JOIN `bandeira` ON `bandeira`.`id` = `loja`.`bandeira`
 WHERE `tributos_mobiliarios`.`id_contratante` = ? GROUP BY `tributos_mobiliarios`.`possui_cnd` 
";
 ...
 $this -> db -> select('*,loja.ins_cnd_mob,tributos_mobiliarios.ativo as status_cnd,tributos_mobiliarios.id as id_cnd,emitente.nome_fantasia as nome');
   ...
   if($tipo <> 'X'){
	...
	$this -> db -> order_by('tributos_mobiliarios.possui_cnd');  
   }
 ...
 function listarTodasCidades(){
   ...
   $this -> db -> join('tributos_mobiliarios','tributos_mobiliarios.id_iptu = iptu.id');   
...
function listarTodasInscricoes($id_contratante){
  ...
  function listarLoja($id_contratante,$tipo){
   $this->db->distinct();
   $this -> db -> select('tributos_mobiliarios.id,emitente.nome_fantasia as  nome');
   $this -> db -> from('loja');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
   $this -> db -> join('tributos_mobiliarios','tributos_mobiliarios.id_loja = loja.id');
   $this -> db -> where('tributos_mobiliarios.id_contratante', $id_contratante);
...
function listarImovelByCidade($id,$tipo){
  ...
  public function add($detalhes = array()){
	...
	if($this->db->insert('tributos_mobiliarios', $detalhes)) {
   ...
   if($query -> num_rows() <> 0){
	...
	$this->db->update('tributos_mobiliarios', $dados); 
...
//print_r($this->db->last_query());exit;
 ...
 function listarInscricaoByLoja($id){
  ...
  $this -> db -> select('*,tributos_mobiliarios.ativo as status_insc,tributos_mobiliarios.id as id_cnd,loja.ins_cnd_mob as ins_cnd_mob,loja.id as id_loja,regional.descricao as desc_regional,bandeira.descricao_bandeira ');
   ...
   $this -> db -> select('*,tributos_mobiliarios.ativo as status_insc,tributos_mobiliarios.id as id_cnd');
 ...
 function listarLojaByImovel($id_loja,$tipo){
   ...
   $this -> db -> select('*,tributos_mobiliarios.ativo as status_insc,tributos_mobiliarios.id as id_cnd,emitente.nome_fantasia as nome,(select distinct loja.id from acomp_cnd where acomp_cnd.id_loja = loja.id) as loja_acomp,loja.id as id_loja');
   $this -> db -> from('tributos_mobiliarios'); 
   $this -> db -> join('loja','tributos_mobiliarios.id_loja = loja.id');
   $this -> db -> join('emitente','emitente.id = loja.id_emitente');
  ...
  function listarImovelByUf($estado,$tipo){
   ...
   $this -> db -> where('tributos_mobiliarios.possui_cnd', '3');  
