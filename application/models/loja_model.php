...
Class Loja_model extends CI_Model{
 ...
 function contarCndByEstado($id_contratante,$estado ){
  ...
  function contarCndByEst($id_contratante,$estado,$tipo ){

	if($estado <> 'BR'){
    if($tipo == 2){
      $sql = "select l.id,l.estado,l.cidade,l.cnpj,n.descricao as descricao_natureza_raiz,DATE_FORMAT(cnt.data_vencto,'%d/%m/%Y') as data_inclusao_sis_ext ,DATE_FORMAT(cnt.data_vencto,'%m') as mes_vencto,DATE_FORMAT(cnt.data_vencto,'%Y') as ano_vencto,DATE_FORMAT(cnt.data_vencto,'%d/%m/%Y') as data_vencto_br  from cnd_mob_tratativa cnt  left join loja l on l.id = cnt.id_cnd_mob  left join tipo_taxas n on n.id = cnt.tipo_taxa  where l.estado=? and l.id_contratante = ? and cnt.tipo_tratativa = ? order by l.cidade";
    }else{
      $sql = "select l.id,l.estado,l.cidade,l.cnpj,n.descricao_natureza_raiz,DATE_FORMAT(cnt.data_inclusao_sis_ext,'%d/%m/%Y') as data_inclusao_sis_ext ,DATE_FORMAT(cnt.data_inclusao_sis_ext,'%m') as mes_vencto,DATE_FORMAT(cnt.data_inclusao_sis_ext,'%Y') as ano_vencto,DATE_FORMAT(cnt.data_vencto,'%d/%m/%Y') as data_vencto_br from cnd_mob_tratativa cnt left join loja l on l.id = cnt.id_cnd_mob left join natureza_raiz n on n.codigo = cnt.id_natureza_raiz where l.estado=? and l.id_contratante = ? and cnt.tipo_tratativa = ? order by l.cidade";
		  
    }
    $query = $this->db->query($sql, array($estado,$id_contratante,$tipo));   
	}else{
    if($tipo == 2){
      $sql = " select l.id,l.estado,l.cidade,l.cnpj,n.descricao as descricao_natureza_raiz,DATE_FORMAT(cnt.data_vencto,'%d/%m/%Y') as data_inclusao_sis_ext ,DATE_FORMAT(cnt.data_vencto,'%m') as mes_vencto,DATE_FORMAT(cnt.data_vencto,'%Y') as ano_vencto,DATE_FORMAT(cnt.data_vencto,'%d/%m/%Y') as data_vencto_br from cnd_mob_tratativa cnt  left join loja l on l.id = cnt.id_cnd_mob left join tipo_taxas n on n.id = cnt.tipo_taxa where l.id_contratante = ? and cnt.tipo_tratativa = ? order by l.cidade";
    }else{
      $sql = "select l.id,l.estado,l.cidade,l.cnpj,n.descricao_natureza_raiz,DATE_FORMAT(cnt.data_inclusao_sis_ext,'%d/%m/%Y') as data_inclusao_sis_ext ,DATE_FORMAT(cnt.data_inclusao_sis_ext,'%m') as mes_vencto,DATE_FORMAT(cnt.data_inclusao_sis_ext,'%Y') as ano_vencto,DATE_FORMAT(cnt.data_vencto,'%d/%m/%Y') as data_vencto_br from cnd_mob_tratativa cnt  left join loja l on l.id = cnt.id_cnd_mob left join natureza_raiz n on n.codigo = cnt.id_natureza_raiz where l.id_contratante = ? and cnt.tipo_tratativa = ? order by l.cidade";
 ...
 function contarCndByStatus($id_contratante,$estado,$tipo ){
	 
	
	if($estado == 'BR'){
		$sql = "SELECT count(*) as total,loja.estado FROM `loja`  left join cnd_mob_tratativa c on c.id_cnd_mob = loja.id      WHERE `loja`.`id_contratante` = '1' and c.tipo_tratativa = $tipo group by loja.estado ";
		$query = $this->db->query($sql, array($id_contratante));    
	}else{
    $sql = "SELECT count(*) as total,loja.estado FROM `loja`  left join cnd_mob_tratativa c on c.id_cnd_mob = loja.id      WHERE `loja`.`id_contratante` = '1' and c.tipo_tratativa = $tipo  and loja.estado = ? group by loja.estado ";
		$query = $this->db->query($sql, array($id_contratante,$estado));   
	}	 
  ...
  function contarCndByMesVencto($id_contratante,$estado,$tipo ){
	 
	
	if($estado == 'BR'){
		$sql = "SELECT count(*) as total,DATE_FORMAT( c.data_vencto,'%m') as mes_vencto FROM `loja`  left join cnd_mob_tratativa c on c.id_cnd_mob = loja.id      WHERE `loja`.`id_contratante` = '1' and c.tipo_tratativa = $tipo group by DATE_FORMAT( c.data_vencto,'%m') ";
		$query = $this->db->query($sql, array($id_contratante));    
	}else{
    $sql = "SELECT count(*) as total,DATE_FORMAT( c.data_vencto,'%m') as mes_vencto  FROM `loja`  left join cnd_mob_tratativa c on c.id_cnd_mob = loja.id      WHERE `loja`.`id_contratante` = '1' and c.tipo_tratativa = $tipo  and loja.estado = ? DATE_FORMAT( c.data_vencto,'%m') ";
		$query = $this->db->query($sql, array($id_contratante,$estado));   
	}	 
